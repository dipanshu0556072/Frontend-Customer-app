import { Alert,StyleSheet, Text, View,TextInput,TouchableOpacity,ActivityIndicator} from 'react-native'
...
const ForgotEmailVerify = ({navigation}) => {
  ...
  async function sendVerifyEmailOtp(){
    // Alert.alert(JSON.stringify(checkEmail)+" "+JSON.stringify(otp));
    try {
      const response = await axios.post(`http://${ip}:5454/auth/reset-password?email=${checkEmail}&otp=${otp}&newPassword=""`);
      console.log(response.data);
      submitEmailOtp();
    } catch (error) {
      console.log("Error occurred while creating user:", error);
