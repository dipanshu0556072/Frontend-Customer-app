...
const ForgotChangePassword = ({navigation}) => {
    ...
    const {ip,checkEmail,token,popFromStack,pushToStack,
      currentPage, setCurrentPage,otp,
       ...
       if(newPassword===''||confirmPassword===''){
         ...
         return;
           ...
           if(newPassword!==confirmPassword){
            ...
            try {
              const response = await axios.post(`http://${ip}:5454/auth/reset-password?email=${checkEmail}&otp=${otp}&newPassword=${newPassword}`, {
                  headers: {
  ...
  return (
    <View style={{flex:1,backgroundColor:'white'}}>
               ...
               <ScrollView showsVerticalScrollIndicator={false}>
                 ...
                 <View style={{marginTop:'18%',flexDirection:'row',alignItems:'center',alignContent:'center'}}>
         ...
         (
           <TouchableOpacity onPress={Close3}>
            ...
            <Text style={{color:'red',alignSelf:'center',margin:'4%'}}>new password & confirm not matched</Text>
            :
            <Text style={{margin:'4%'}}></Text>
          }
          {
            !currentPasswordBackendMatched ?
              <Text style={{textAlign:'center',color:'red'}}>current password is not correct</Text>
            :
            <Text></Text>
          }
